...
**Domain:** `ESG / Report Parsing` · **Suite:** `General Automation Suite` · **Accent:** `#93c5fd`
...
**Esg Report Parser** is a standalone, browser-based AI workflow app for **ESG / Report Parsing**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.

**Core job:** Sustainability PDF → normalized ESG metrics.
...
|---|---:|:---:|---|---|
| `sustainability_pdf`<br>Sustainability PDF | text | Yes | — | Affects input: Sustainability PDF. |
...
| `framework`<br>framework | select | No | auto detect | Affects customization: framework. |
| `industry`<br>industry | text | No | — | Affects customization: industry. |
| `reporting_year`<br>reporting year | text | No | — | Affects customization: reporting year. |
| `units`<br>units | text | No | — | Affects customization: units. |
| `strictness`<br>strictness | slider | No | 50 | Affects customization: strictness. |
| `evidence_requirement`<br>evidence requirement | text | No | — | Affects customization: evidence requirement. |
| `output_schema`<br>output schema | select | No | — | Affects customization: output schema. |
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9120`. |
...
- Export download
- Job history
- Framework/factor database optional

### Real Connector Requirements

- emissions factor source
- ESG reporting framework reference
...
```bash
curl -X POST http://127.0.0.1:9120/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9120 uvicorn app.main:app --host 0.0.0.0 --port 9120
```
...
- `plugins/esg-report-parser.json` for plugin metadata.
